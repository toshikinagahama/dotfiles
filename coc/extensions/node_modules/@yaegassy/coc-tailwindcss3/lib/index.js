...
function sortedWorkspaceFolders() {
  if (_sortedWorkspaceFolders === void 0) {
    _sortedWorkspaceFolders = import_coc2.workspace.workspaceFolders ? import_coc2.workspace.workspaceFolders.map((folder) => {
...
function getOuterMostWorkspaceFolder(folder) {
  ...
  for (const element of sorted) {
    ...
    if (uri.startsWith(element)) {
      const workdir = import_coc2.workspace.getWorkspaceFolder(element);
      if (workdir) {
        return workdir;
      }
    }
...
function getUserLanguages(folder) {
  const langs = folder ? import_coc2.workspace.getConfiguration("tailwindCSS", folder.uri.toString()).includeLanguages : void 0;
  return isObject(langs) ? langs : {};
}
function isExcluded(file, folder) {
  const exclude = getConfigExcludePatterns();
  for (const pattern of exclude) {
    if ((0, import_minimatch.default)(file, import_path.default.join(import_coc2.Uri.parse(folder.uri).fsPath, pattern))) {
      return true;
    }
  }
  return false;
...
async function activate(context) {
  ...
  if (module2 && import_fs.default.existsSync(module2)) {
    ...
    module2 = context.asAbsolutePath(import_path.default.join("node_modules", "@tailwindcss", "language-server", "bin", "tailwindcss-language-server"));
  }
  const outputChannel = import_coc2.window.createOutputChannel("tailwindcss-language-server");
  context.subscriptions.push(import_coc2.commands.registerCommand("tailwindCSS.showOutput", () => {
    if (outputChannel) {
      outputChannel.show();
    }
  }));
  const watcher = import_coc2.workspace.createFileSystemWatcher(`**/${CONFIG_FILE_GLOB}`, false, true, true);
  watcher.onDidCreate((uri) => {
    let folder = import_coc2.workspace.getWorkspaceFolder(uri.toString());
    if (!folder) {
      return;
    }
  ...
  context.subscriptions.push(import_coc2.workspace.onDidChangeConfiguration((event) => {
    clients.forEach((client, key) => {
      const folder = import_coc2.workspace.getWorkspaceFolder(import_coc2.Uri.parse(key).toString());
      if (!folder)
        return;
  ...
  function bootWorkspaceClient(folder) {
    ...
    const configuration = {
      edidor: import_coc2.workspace.getConfiguration("editor"),
      tailwindCSS: import_coc2.workspace.getConfiguration("tailwindCSS")
    };
    ...
    const clientOptions = {
      documentSelector: languageObj.map((language) => ({
        scheme: "file",
        language,
        pattern: `${import_coc2.Uri.parse(folder.uri).fsPath}/**/*`
      })),
      diagnosticCollectionName: "tailwindcss-language-server",
      workspaceFolder: folder,
      outputChannel,
      synchronize: {
        fileEvents: import_coc2.workspace.createFileSystemWatcher(CONFIG_FILE_GLOB)
      },
      middleware: {
        workspace: {
          configuration: (params) => {
            return params.items.map(({ section, scopeUri }) => {
              let scope = folder;
              if (scopeUri) {
                const doc = import_coc2.workspace.textDocuments.find((doc2) => doc2.uri.toString() === scopeUri);
                if (doc) {
                  scope = {
              ...
              return import_coc2.workspace.getConfiguration(section);
            });
          }
        }
      },
    ...
    const client = new import_coc2.LanguageClient("tailwindCSS", "Tailwind CSS Language Server", serverOptions, clientOptions);
    client.start();
    clients.set(folder.uri.toString(), client);
  }
  async function didOpenTextDocument(document) {
    const uri = import_coc2.Uri.parse(document.uri);
    if (uri.scheme !== "file")
      return;
    let folder = import_coc2.workspace.getWorkspaceFolder(document.uri);
    if (!folder) {
      return;
    }
    folder = getOuterMostWorkspaceFolder(folder);
    const [configFile] = await import_coc2.workspace.findFiles(new import_coc2.RelativePattern(folder, `**/${CONFIG_FILE_GLOB}`), `{${getConfigExcludePatterns().join(",")}}`, 1);
    if (!configFile)
      return;
    bootWorkspaceClient(folder);
  }
  context.subscriptions.push(import_coc2.workspace.onDidOpenTextDocument(didOpenTextDocument));
  import_coc2.workspace.textDocuments.forEach(didOpenTextDocument);
  context.subscriptions.push(import_coc2.workspace.onDidChangeWorkspaceFolders((event) => {
